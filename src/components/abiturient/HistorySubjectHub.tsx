...
import { BookText, ChevronLeft, PenTool, ScrollText } from "lucide-react";
...
/** History Space hub — the history equivalent of GeorgianSubjectHub. The real
 *  exam is test questions plus a source-based open task, so it gets two cards:
 *  reading comprehension and the written task. Colours: violet (primary),
 *  amber (secondary). */
export function HistorySubjectHub() {
  return (
    <main className="relative mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 sm:py-10">
      <Sparkle className="pointer-events-none absolute left-1/3 top-4 hidden h-4 w-4 -rotate-12 text-violet-400 opacity-70 xl:block" />
      ...
      <header className="mb-9 flex flex-col gap-4 sm:flex-row sm:items-center">
        ...
        >
          <ScrollText className="h-7 w-7 stroke-[2]" aria-hidden />
        </span>
        <div>
      ...
      >
        <SubjectExerciseCard
          ...
          description="შემთხვევით ამოსული ტესტური კითხვა (ზოგი წყაროზე დაფუძნებული) წინა წლების რეალური გამოცდებიდან. აირჩიე პასუხი და მაშინვე ნახე სწორი პასუხი და ახსნა."
          icon={<BookText className="h-10 w-10 stroke-[1.75]" aria-hidden />}
          btnText="ტესტის დაწყება"
          href="/subject/history/reading-comprehension"
          accent="violet"
          tilt="hover:-rotate-1"
        ...
        <SubjectExerciseCard
          ...
          description="უპასუხე შემთხვევით ამოსულ წყაროზე დაფუძნებულ ღია დავალებას და მიიღე AI შეფასება შეფასების სქემის კრიტერიუმების მიხედვით."
          icon={<PenTool className="h-10 w-10 stroke-[1.75]" aria-hidden />}
          btnText="დაწყება"
          href="/subject/history/essay-practice"
          accent="amber"
          tilt="hover:rotate-1"
          corner={<Flower className="h-9 w-9 -rotate-12 text-amber-400/80" />}
        />
      </section>
    </main>
  );
