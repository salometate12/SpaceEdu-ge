...
import { ChevronLeft, Landmark, ListChecks, PenTool } from "lucide-react";
...
/** Civics Space hub — the civics equivalent of MathSubjectHub. Two cards: the 24
 *  test questions and the AI-graded open reasoning tasks. Colours: pink, violet. */
export function CivicsSubjectHub() {
  return (
    <main className="relative mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 sm:py-10">
      <Sparkle className="pointer-events-none absolute left-1/3 top-4 hidden h-4 w-4 -rotate-12 text-pink-400 opacity-70 xl:block" />
      ...
      <header className="mb-9 flex flex-col gap-4 sm:flex-row sm:items-center">
        ...
        >
          <Landmark className="h-7 w-7 stroke-[2]" aria-hidden />
        </span>
        <div>
          <h1 className="headline text-3xl font-bold text-slate-900 dark:text-slate-50">სამოქალაქო განათლება</h1>
      ...
      <section className="grid grid-cols-1 gap-5 md:grid-cols-2" aria-label="საგამოცდო მოდულები">
        <SubjectExerciseCard
          ...
          description="შემთხვევით ამოსული ტესტური კითხვა წინა წლების რეალური გამოცდებიდან. აირჩიე პასუხი და მაშინვე ნახე სწორია თუ არა, ახსნით."
          icon={<ListChecks className="h-10 w-10 stroke-[1.75]" aria-hidden />}
          btnText="დაწყება"
          href="/subject/civics/mcq-practice"
          accent="pink"
          tilt="hover:-rotate-1"
        ...
        <SubjectExerciseCard
          ...
          description="უპასუხე შემთხვევით ამოსულ ღია დავალებას (მსჯელობა, არგუმენტაცია, წყაროს ანალიზი) და მიიღე AI შეფასება შეფასების სქემის კრიტერიუმებით."
          icon={<PenTool className="h-10 w-10 stroke-[1.75]" aria-hidden />}
          btnText="დაწყება"
          href="/subject/civics/open-practice"
          accent="violet"
          tilt="hover:rotate-1"
          corner={<Flower className="h-9 w-9 -rotate-12 text-violet-400/80" />}
        />
      </section>
    </main>
  );
