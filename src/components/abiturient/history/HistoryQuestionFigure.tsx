...
/**
 * A question's figure — for history that means the full historical map that
 * 36.1–36.6 read from. Unlike the small maths diagrams, these are detailed
 * maps with fine legend text, so it renders large and opens a full-screen
 * lightbox on click so the legend is actually readable.
 */
export function HistoryQuestionFigure({
  figure,
}: {
  ...
  // Let Escape close the lightbox, and lock body scroll while it's open.
  useEffect(() => {
    if (!zoomed) return;
  ...
  return (
    <>
      <button
        ...
        className="group relative mt-1 block w-full overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-white/10 dark:bg-[#0c0b12]"
        aria-label="რუკის გადიდება"
      >
