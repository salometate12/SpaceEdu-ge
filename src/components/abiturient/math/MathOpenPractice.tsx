...
import { MathText, MathBlock } from "@/components/abiturient/exams/MathText";
import { useMathOpenGrading } from "@/components/abiturient/exams/useMathOpenGrading";
import { MathOpenProblemReport } from "@/components/abiturient/math/MathOpenProblemReport";
import {
  mathPaperLabel,
  pickRandomMathOpenProblem,
  type MathOpenDraw,
} from "@/lib/math-past-paper-practice";
import {
  ACCENT_CARD,
  ACCENT_PILL,
  ACCENT_SOLID,
...
export function MathOpenPractice() {
  // Client-only (ssr:false), so the random draw on first render is safe.
  const [draw, setDraw] = useState<MathOpenDraw | null>(() => pickRandomMathOpenProblem());
  const [answer, setAnswer] = useState("");
  const grading = useMathOpenGrading("abit-math-open-practice");

  const reroll = () => {
    setDraw(pickRandomMathOpenProblem(draw?.problem.id));
    setAnswer("");
    grading.reset();
  };

  const p = draw?.problem;
      ...
      {!p || !draw ? (
        <div className={`rounded-2xl border-2 p-10 text-center ${PLAIN_CARD}`}>
          <p className="text-sm text-slate-700 dark:text-slate-300">
            ღია ამოცანების ბანკი ჯერ მზადდება.
          </p>
        </div>
      ) : (
        <>
          <section className={`rounded-2xl border-2 p-6 ${ACCENT_CARD[ACCENT]}`}>
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                ღია ამოცანა · {p.number} · {p.points} ქულა
              </span>
              <span
            ...
            <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-5 dark:border-white/10 dark:bg-[#121214]">
              ...
              {p.figure && (
                ...
                <img src={p.figure.src} alt={p.figure.alt} className="my-3 max-h-60 w-auto" />
              )}
            </div>
          ...
          <section className="mt-4 rounded-2xl border-2 border-slate-300/80 bg-white/60 p-5 shadow-sm sm:p-6 dark:border-sky-400/20 dark:bg-white/[0.05]">
            ...
            <textarea
              ...
              className="mt-2 min-h-[200px] w-full resize-y rounded-2xl border-2 border-slate-300/80 bg-white p-4 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-sky-500/70 dark:border-white/[0.14] dark:bg-[#0c0b12] dark:text-slate-100 dark:placeholder:text-slate-500"
            />

            <button
              type="button"
              onClick={() => void grading.grade(p, answer)}
