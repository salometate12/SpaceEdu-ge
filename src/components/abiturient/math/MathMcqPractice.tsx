...
export function MathMcqPractice() {
  // Drawn on the first render. The component is client-only (loaded with
  // ssr:false), so the server never commits to a question and the markup and
  // hydration can't disagree on the random pick.
  const [draw, setDraw] = useState<MathMcqDraw | null>(() => pickRandomMathMcq());
  const [picked, setPicked] = useState<string | null>(null);
  const [streak, setStreak] = useState(0);

  const reroll = () => {
    setDraw(pickRandomMathMcq(draw?.question.id));
    setPicked(null);
  };
  ...
  const onPick = (label: string) => {
    ...
    setPicked(label);
    const correct = label === draw.question.correctLabel;
      ...
      {!q || !draw ? (
        <div className={`rounded-2xl border-2 p-10 text-center ${PLAIN_CARD}`}>
          <p className="text-sm text-slate-700 dark:text-slate-300">
            ტესტური კითხვების ბანკი ჯერ მზადდება.
          </p>
        </div>
      ) : (
        <section className={`rounded-2xl border-2 p-6 ${ACCENT_CARD[ACCENT]}`}>
          ...
          <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-5 dark:border-white/10 dark:bg-[#121214]">
            ...
            {q.figure && (
              ...
              <img src={q.figure.src} alt={q.figure.alt} className="my-3 max-h-60 w-auto" />
            )}
              ...
              {q.options.map((opt) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-2 rounded-xl border-2 px-3 py-2 text-left text-sm transition disabled:cursor-default ${
                      isCorrect
                        ...
                        : isWrongPick
                          ...
                          : "border-slate-200 hover:border-sky-300 dark:border-white/10 dark:hover:border-sky-400/40"
                    }`}
                  >
