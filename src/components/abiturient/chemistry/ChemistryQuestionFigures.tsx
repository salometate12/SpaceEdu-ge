...
import type { ChemistryFigure } from "@/data/chemistryExamsData";

/** One figure with a full-screen zoom lightbox — chemistry structures and
 *  reaction schemes carry fine detail, so they render large and open on click. */
function FigureView({ figure }: { figure: ChemistryFigure }) {
  ...
  return (
    <>
      <button
        ...
        className="group relative block w-full overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-white/10 dark:bg-white"
        aria-label="სურათის გადიდება"
      >
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src={figure.src} alt={figure.alt} className="max-h-[70vh] w-full bg-white object-contain" />
      ...
      {zoomed && (
        ...
        >
          ...
          <img
            ...
            className="max-h-[95vh] max-w-[95vw] cursor-default rounded-lg bg-white object-contain shadow-2xl"
          />
        </div>
      )}
    </>
...
export function ChemistryQuestionFigures({ figures }: { figures?: ChemistryFigure[] }) {
