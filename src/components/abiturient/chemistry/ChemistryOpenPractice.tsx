...
import { gradeChemistryOpenSubItem } from "@/components/abiturient/exams/useChemistryOpenGrading";
...
import {
  chemistryPaperLabel,
  pickRandomChemistryOpen,
  type ChemistryOpenDraw,
...
export function ChemistryOpenPractice() {
  const [draw, setDraw] = useState<ChemistryOpenDraw | null>(() => pickRandomChemistryOpen());
  const [answer, setAnswer] = useState("");
  const [report, setReport] = useState<ChemistryOpenGraderReport | null>(null);
  const [usedFallback, setUsedFallback] = useState(false);
  const [busy, setBusy] = useState(false);

  const reroll = () => {
    setDraw(pickRandomChemistryOpen(draw?.subItem.id));
    setAnswer("");
    setReport(null);
  ...
  const grade = async () => {
    ...
    setBusy(true);
    setReport(null);
    recordToolUsage("abit-chemistry-open-practice", "ქიმიის ღია დავალების შემფასებელი");
    try {
      const outcome = await gradeChemistryOpenSubItem(draw.subItem, draw.task, answer);
      ...
      {!subItem || !task || !draw ? (
        ...
        <>
          <section className={`rounded-2xl border-2 p-6 ${ACCENT_CARD[ACCENT]}`}>
            ...
            <ChemistryQuestionFigures figures={task.figures ?? subItem.figures} />
