...
import { ArrowLeft, CalendarRange, Check, Dices, Flame, X } from "lucide-react";
...
import {
  chemistryPaperLabel,
  pickRandomChemistryMcq,
  type ChemistryMcqDraw,
} from "@/lib/chemistry-past-paper-practice";
import {
  ACCENT_CARD,
  ACCENT_PILL,
  ACCENT_SOLID,
...
export function ChemistryMcqPractice() {
  const [draw, setDraw] = useState<ChemistryMcqDraw | null>(() => pickRandomChemistryMcq());
  const [picked, setPicked] = useState<string | null>(null);
  const [streak, setStreak] = useState(0);

  const reroll = () => {
    setDraw(pickRandomChemistryMcq(draw?.question.id));
    setPicked(null);
  };
    ...
    <main className="relative mx-auto w-full max-w-3xl px-4 py-6 sm:px-6 sm:py-8">
      ...
      {!q || !draw ? (
        <div className={`rounded-2xl border-2 p-10 text-center ${PLAIN_CARD}`}>
          <p className="text-sm text-slate-700 dark:text-slate-300">ტესტური კითხვების ბანკი ჯერ მზადდება.</p>
        </div>
      ) : (
        <section className={`rounded-2xl border-2 p-6 ${ACCENT_CARD[ACCENT]}`}>
          ...
          <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-5 dark:border-white/10 dark:bg-[#121214]">
            <p className="text-[15px] font-medium leading-relaxed text-slate-900 dark:text-slate-100">{q.prompt}</p>
            ...
            <div className="mt-3 space-y-2">
              {q.options.map((opt) => {
                ...
                const isPicked = !revealed && picked === opt.label;
                return (
                  <button
                    key={opt.label}
                    type="button"
                    ...
                    className={`flex w-full items-start gap-2 rounded-xl border-2 px-3 py-2 text-left text-sm transition disabled:cursor-default ${
                      isCorrect
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1 text-slate-800 dark:text-slate-200">{opt.text || "(იხ. სურათი)"}</span>
            ...
            {revealed && (
              <>
                ...
                <p className="mt-2 rounded-xl border border-slate-200 bg-slate-50 p-3 text-[13px] leading-relaxed text-slate-600 dark:border-white/10 dark:bg-white/[0.03] dark:text-zinc-300">
                  {q.explanation}
                </p>
              </>
            )}
