...
import { ChevronLeft, Globe2, ListChecks, PenTool } from "lucide-react";
...
/** Geography Space hub — the geography equivalent of MathSubjectHub. Two cards:
 *  the 27 test questions (with maps/charts/photos) and the AI-graded open tasks.
 *  Colours: green (primary), amber (secondary). */
export function GeographySubjectHub() {
  return (
    <main className="relative mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 sm:py-10">
      ...
      <header className="mb-9 flex flex-col gap-4 sm:flex-row sm:items-center">
        ...
        >
          <Globe2 className="h-7 w-7 stroke-[2]" aria-hidden />
        </span>
        <div>
          <h1 className="headline text-3xl font-bold text-slate-900 dark:text-slate-50">გეოგრაფია</h1>
      ...
      <section className="grid grid-cols-1 gap-5 md:grid-cols-2" aria-label="საგამოცდო მოდულები">
        <SubjectExerciseCard
          ...
          description="შემთხვევით ამოსული ტესტური კითხვა წინა წლების რეალური გამოცდებიდან — რუკებით, დიაგრამებით და ფოტოებით. აირჩიე პასუხი და მაშინვე ნახე სწორია თუ არა, ახსნით."
          icon={<ListChecks className="h-10 w-10 stroke-[1.75]" aria-hidden />}
          btnText="დაწყება"
          href="/subject/geography/mcq-practice"
          accent="green"
          tilt="hover:-rotate-1"
        ...
        <SubjectExerciseCard
          ...
          description="უპასუხე შემთხვევით ამოსულ ღია დავალებას (გამოთვლა ან წყაროზე დაფუძნებული ახსნა) და მიიღე AI შეფასება შეფასების სქემის კრიტერიუმებით."
          icon={<PenTool className="h-10 w-10 stroke-[1.75]" aria-hidden />}
          btnText="დაწყება"
          href="/subject/geography/open-practice"
          accent="amber"
          tilt="hover:rotate-1"
