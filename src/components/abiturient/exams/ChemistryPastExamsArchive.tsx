...
import {
  getChemistryExamYears,
  getChemistryVariant,
  type ChemistryExamVariant,
} from "@/data/chemistryExamsData";
import { ChemistryExamSimulation } from "./ChemistryExamSimulation";

/** The chemistry equivalent of PastExamsArchive: same year/variant flow, runner
 *  is ChemistryExamSimulation and the data is CHEMISTRY_EXAM_YEARS. */
export function ChemistryPastExamsArchive() {
  const years = useMemo(() => getChemistryExamYears(), []);
  const [activeYear, setActiveYear] = useState<number | null>(years[0]?.year ?? null);
  const [runId, setRunId] = useState<string | null>(null);

  const runVariant = runId ? getChemistryVariant(runId) : null;
  const selectedYear = years.find((y) => y.year === activeYear) ?? years[0] ?? null;

  if (runVariant) {
    return <ChemistryExamSimulation variant={runVariant} onExit={() => setRunId(null)} />;
  }
  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-4 py-6 sm:px-6 sm:py-8">
      ...
      <header className="rounded-2xl border-2 border-teal-300/70 bg-teal-100/50 p-6 dark:border-teal-400/25 dark:bg-teal-400/[0.07]">
        <span className="inline-flex items-center gap-1.5 rounded-full border-2 border-teal-400/70 bg-teal-100/70 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-teal-700 dark:border-teal-400/30 dark:bg-teal-400/10 dark:text-teal-200">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate-700 dark:text-slate-300">
          რეალური საგამოცდო ვარიანტები — ტესტური კითხვები (სტრუქტურებით, სქემებით) და ღია დავალებები:
          ფორმულები, განტოლებები და გამოთვლები. იმიტირებული გამოცდა: პასუხები არსად ჩანს დასრულებამდე,
          ბოლოს კი ღია დავალებებს AI აფასებს შეფასების სქემის კრიტერიუმებით.
        </p>
      </header>
                ...
                return (
                  <button
                    ...
                    className={`rounded-2xl border-2 px-5 py-3 text-lg font-bold transition ${
                      active
                        ? "border-teal-400/70 bg-teal-100/70 text-teal-800 dark:border-teal-400/40 dark:bg-teal-400/10 dark:text-teal-200"
          ...
          {selectedYear && (
            <section className="mt-8">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {selectedYear.variants.map((variant: ChemistryExamVariant) => {
                  ...
                  return (
                    ...
                    >
                      <span className="inline-flex items-center gap-1.5 rounded-full border-2 border-teal-400/70 bg-teal-100/70 px-3.5 py-1 text-[11px] font-bold text-teal-700 dark:border-teal-400/30 dark:bg-teal-400/10 dark:text-teal-200">
                      ...
                      <span className="mt-4 inline-flex items-center gap-1.5 text-xs font-bold text-teal-700 dark:text-teal-300">
