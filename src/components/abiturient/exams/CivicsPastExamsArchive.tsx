...
import { CivicsExamSimulation } from "./CivicsExamSimulation";

/** The civics equivalent of PastExamsArchive: same year/variant flow, runner is
 *  CivicsExamSimulation and the data is CIVICS_EXAM_YEARS. */
export function CivicsPastExamsArchive() {
  const years = useMemo(() => getCivicsExamYears(), []);
  const [activeYear, setActiveYear] = useState<number | null>(years[0]?.year ?? null);
  const [runId, setRunId] = useState<string | null>(null);

  const runVariant = runId ? getCivicsVariant(runId) : null;
  const selectedYear = years.find((y) => y.year === activeYear) ?? years[0] ?? null;

  if (runVariant) {
    return <CivicsExamSimulation variant={runVariant} onExit={() => setRunId(null)} />;
  }
  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-4 py-6 sm:px-6 sm:py-8">
      ...
      <header className="rounded-2xl border-2 border-rose-300/70 bg-rose-100/50 p-6 dark:border-rose-400/25 dark:bg-rose-400/[0.07]">
        <span className="inline-flex items-center gap-1.5 rounded-full border-2 border-rose-400/70 bg-rose-100/70 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-rose-700 dark:border-rose-400/30 dark:bg-rose-400/10 dark:text-rose-200">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate-700 dark:text-slate-300">
          რეალური საგამოცდო ვარიანტები — ტესტური კითხვები, შესაბამისობა, შეცდომის გასწორება,
          წყაროსა და ილუსტრაციის ანალიზი. იმიტირებული გამოცდა: პასუხები არსად ჩანს დასრულებამდე,
                ...
                return (
                  <button
                    ...
                    className={`rounded-2xl border-2 px-5 py-3 text-lg font-bold transition ${
                      active
                        ? "border-rose-400/70 bg-rose-100/70 text-rose-800 dark:border-rose-400/40 dark:bg-rose-400/10 dark:text-rose-200"
          ...
          {selectedYear && (
            <section className="mt-8">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {selectedYear.variants.map((variant: CivicsExamVariant) => {
                  ...
                  return (
                    ...
                    >
                      <span className="inline-flex items-center gap-1.5 rounded-full border-2 border-rose-400/70 bg-rose-100/70 px-3.5 py-1 text-[11px] font-bold text-rose-700 dark:border-rose-400/30 dark:bg-rose-400/10 dark:text-rose-200">
                      ...
                      <span className="mt-4 inline-flex items-center gap-1.5 text-xs font-bold text-rose-700 dark:text-rose-300">
