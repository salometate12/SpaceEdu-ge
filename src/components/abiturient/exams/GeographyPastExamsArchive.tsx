...
import {
  getGeographyExamYears,
  getGeographyVariant,
  type GeographyExamVariant,
} from "@/data/geographyExamsData";
import { GeographyExamSimulation } from "./GeographyExamSimulation";

/** The geography equivalent of PastExamsArchive: same year/variant flow, but the
 *  runner is GeographyExamSimulation and the data is GEOGRAPHY_EXAM_YEARS. */
export function GeographyPastExamsArchive() {
  const years = useMemo(() => getGeographyExamYears(), []);
  const [activeYear, setActiveYear] = useState<number | null>(years[0]?.year ?? null);
  const [runId, setRunId] = useState<string | null>(null);

  const runVariant = runId ? getGeographyVariant(runId) : null;
  const selectedYear = years.find((y) => y.year === activeYear) ?? years[0] ?? null;

  if (runVariant) {
    return <GeographyExamSimulation variant={runVariant} onExit={() => setRunId(null)} />;
  }
  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-4 py-6 sm:px-6 sm:py-8">
      ...
      <header className="rounded-2xl border-2 border-emerald-300/70 bg-emerald-100/50 p-6 dark:border-emerald-400/25 dark:bg-emerald-400/[0.07]">
        <span className="inline-flex items-center gap-1.5 rounded-full border-2 border-emerald-400/70 bg-emerald-100/70 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-emerald-700 dark:border-emerald-400/30 dark:bg-emerald-400/10 dark:text-emerald-200">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate-700 dark:text-slate-300">
          რეალური საგამოცდო ვარიანტები — ტესტური კითხვები (რუკებით, დიაგრამებით, ფოტოებით) და
          წყაროზე დაფუძნებული ღია დავალებები. იმიტირებული გამოცდა: პასუხები არსად ჩანს დასრულებამდე,
          ბოლოს კი ღია დავალებებს AI აფასებს შეფასების სქემის კრიტერიუმების მიხედვით.
        </p>
      </header>
                ...
                return (
                  <button
                    ...
                    className={`rounded-2xl border-2 px-5 py-3 text-lg font-bold transition ${
                      active
                        ? "border-emerald-400/70 bg-emerald-100/70 text-emerald-800 dark:border-emerald-400/40 dark:bg-emerald-400/10 dark:text-emerald-200"
          ...
          {selectedYear && (
            <section className="mt-8">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {selectedYear.variants.map((variant: GeographyExamVariant) => {
                  ...
                  return (
                    ...
                    >
                      <span className="inline-flex items-center gap-1.5 rounded-full border-2 border-emerald-400/70 bg-emerald-100/70 px-3.5 py-1 text-[11px] font-bold text-emerald-700 dark:border-emerald-400/30 dark:bg-emerald-400/10 dark:text-emerald-200">
                      ...
                      <span className="mt-4 inline-flex items-center gap-1.5 text-xs font-bold text-emerald-700 dark:text-emerald-300">
